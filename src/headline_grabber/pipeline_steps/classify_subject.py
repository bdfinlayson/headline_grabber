...
from src.headline_grabber.pipeline_steps import subject_classification_model, subject_classification_tokenizer
...
class ClassifySubject(PipelineStep):
    ...
    def run(self, context: PipelineContext):
        classifier = pipeline("text-classification", model=subject_classification_model, tokenizer=subject_classification_tokenizer)
        texts = [' '.join([headline.title, headline.description]) for headline in context.headlines]
        results = classifier(texts)
        context.headlines = [headline.set_subject_classification(Classification(self.subject_class_label_mapping[result['label']], result['score'])) for headline, result in zip(context.headlines, results)]
        return context
